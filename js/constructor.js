...
class Food {
  constructor(name, servingSize, calories, protein, carbs, fats) {
    this.name = name;
    this.servingSize = servingSize;
    this.calories = calories;
    this.protein = protein;
...
// Define food objects (assuming `Food` class is defined as above)
const foods = {
  chicken: new Food("Chicken", 85, 128, 26, 0, 2.7),
  rice: new Food("Rice", 120, 160, 3, 35, 0),
  beef: new Food("Lean Beef 96/4", 88, 140, 24, 0, 5),
  ...
  chickenPatty: new Food("Chicken Patty", 85, 170, 12, 15, 7),
  spicyKetchup: new Food("Spicy Ketchup", 17, 20, 0, 4, 0),
  optimumNutritionChocolate: new Food("Optimum Nutrition Extreme Milk Chocolate Protein", 32, 120, 24, 3, 2),
  borrachoBeans: new Food("H-E-B Borracho Beans", 130, 90, 5, 15, 1.5),
  blackBeans: new Food("H-E-B Black Beans", 130, 100, 7, 19, 0),
  crinkleCutFries: new Food("H-E-B Crinkle Cut Fries", 84, 100, 2, 16, 3),
  gardenSaladBlend: new Food("H-E-B Garden Salad", 85, 15, 1, 3, 0),
  lighBalsamic: new Food("H-E-B Light Balsamic", 30, 15, 0, 2, 0.5),
  gratedParmesan: new Food("H-E-B Grated Parmesan", 5, 20, 2, 0, 1.5)
};

// Convert the foods object to a JSON-compatible structure
const foodsJson = JSON.stringify(
  Object.fromEntries(
    Object.entries(foods).map(([key, food]) => [key, food.toJSON()])
  ),
  null,
  2 // Pretty-print the JSON
);

// Output the JSON to console or write to a file
console.log(foodsJson);

const fs = require('fs');

// Save JSON to a file
fs.writeFile('foods.json', foodsJson, (err) => {
  if (err) {
